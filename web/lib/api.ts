...
function createCrudService<T extends { id: number }, TCreate, TUpdate = T>(resource: string): CrudService<T, TCreate, TUpdate> {
  return {
    list: () => request<T[]>(resource), get: id => request<T>(`${resource}/${id}`),
    ...
    remove: async id => { await request<unknown>(`${resource}/${id}`, { method: "DELETE" }); },
  };
}
...
export const ownerService = createCrudService<Owner, Pick<Owner, "firstName" | "lastName" | "gym" | "countryId">>("Owner");
